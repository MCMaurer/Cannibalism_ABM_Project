;; TODO
;; think about how long a tick is gonna be. Have to think about hour vs. day, maybe half-day? Most relevant things happen on day scale, except
;; maybe movement.
...
to inf-death
  if random-float 100 < inf_death_odds [die]
end
...
to inf-juv-death
  if random-float 100 < inf_juv_death_odds [die]
end
...
to can-move
  if random-float 100 < can_move_odds [setxy xcor + random 2 - random 2 ycor + random 2 - random 2]
end

to juv-move
  if random-float 100 < juv_move_odds [setxy xcor + random 2 - random 2 ycor + random 2 - random 2]
end
...
;; trying new infection procedure
to infect
  let infectees uninfecteds-on neighbors
  if infectees != nobody[
  ask infectees[
        if random-float 1000 < infectious_level [
          hatch-infecteds 1 [set color red] die]]]
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; if environmental transmission odds > 0, then you'll get some level of environmental transmission from a reservoir
;; it might make sense to make environmental transmission a function of the number of infecteds, with some lag
;; could also make reservoirs explicitly spatial, where an area an infected has been has some level of environmental transmission capability for
;; some amount of time after they leave

to env-infection
  ask uninfecteds[
  if random-float 100 < env_trans[
    hatch-infecteds 1 [set color red] die ]]
...
to inf-reproduce
  ifelse (((count uninfecteds-on neighbors) + (count infecteds-on neighbors)) != 0)
    [if random-float 100 < (inf-fecundity) * (2 / ((count uninfecteds-on neighbors) + (count infecteds-on neighbors))) ; this used to be 1/ but I'm gonna try it as 2/
      [ifelse random-float 100 < vert_trans_odds
        [hatch-inf-juveniles 1 [set color magenta]]
        [hatch-juveniles 1 [set color pink]]]]
    [if random-float 100 < (inf-fecundity)
      [ifelse random-float 100 < vert_trans_odds
        [hatch-inf-juveniles 1 [set color magenta]]
        [hatch-juveniles 1 [set color pink]]]]
...
to inf-mature
  if development-level >= maturation_time + inf_mature_time [
  hatch-infecteds 1 [set color red] die ]
  ;; nymphs reach reproductive age
end
...
509
Mean Adults/Patch
mean [count infecteds-here] of patches + mean [count uninfecteds-here] of patches
2
1
11
...
459
Max Juvs/Patch
max [count juveniles-here] of patches + max [count inf-juveniles-here] of patches
2
1
11
...
<experiments>
  <experiment name="test" repetitions="100" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="can_move_odds">
      <value value="7.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="infectious_level">
      <value value="70"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="juv_move_odds">
      <value value="0.5"/>
    </enumeratedValueSet>
  ...
  <experiment name="ChangeCannodds" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="9000"/>
    <exitCondition>ticks &gt; 1800 and count infecteds = 0</exitCondition>
    ...
    <steppedValueSet variable="can_move_odds" first="1" step="0.1" last="5"/>
    <enumeratedValueSet variable="juv_death_odds">
      <value value="84"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial_number_uninfecteds">
  ...
  <experiment name="sensitivity_infected-death-odds" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>count uninfecteds</metric>
    ...
    <steppedValueSet variable="inf_death_odds" first="0" step="0.025" last="0.5"/>
    <enumeratedValueSet variable="juv_death_odds">
      <value value="1.2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="juv_move_odds">
    ...
    <enumeratedValueSet variable="inf_juv_death_odds">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial_number_uninfecteds">
      <value value="220"/>
    ...
    <enumeratedValueSet variable="inf_mature_time">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="inf_fecund_odds">
      <value value="-1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="vertical_transmission">
      <value value="false"/>
  ...
  <experiment name="sensitivity_inf_fecund_odds" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <metric>count uninfecteds</metric>
    ...
    <enumeratedValueSet variable="inf_death_odds">
      <value value="0.3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="juv_death_odds">
      <value value="1.2"/>
    ...
    <steppedValueSet variable="inf_fecund_odds" first="-4" step="0.25" last="0"/>
    <enumeratedValueSet variable="vertical_transmission">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="cannibalism_level">
