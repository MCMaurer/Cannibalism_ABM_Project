...
# for this model run, I will be using a narrower parameter range, with single reasonable values for the demographic parameters, since they are massively influential but also something you could more easily know for a given system.

# so the goal here is to say "if you know reasonable demographic rates for a system, how much does cannibalism affect things?
...
write("<experiments>",file="single_demographic_parameter_simulations/netlogo_param_file.xml",append=T)
for(i in 1:length(testParams$V1)){
  ...
  write(line,file="single_demographic_parameter_simulations/netlogo_param_file.xml",append=T)
}
write("</experiments>",file="single_demographic_parameter_simulations/netlogo_param_file.xml",append=T)
