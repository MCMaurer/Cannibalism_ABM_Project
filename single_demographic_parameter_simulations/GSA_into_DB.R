...
directory <- "../../../../Volumes/g_drive_ssd_1/Cann_ABM/"
...
db_location <- "../../../../Volumes/g_drive_ssd_1/Cann_ABM/Cann_ABM_single_demo_param_sim.sqlite3"

if (!file.exists(db_location)) {
  mydb <- src_sqlite(db_location, create = T)
...
con <- DBI::dbConnect(RSQLite::SQLite(), dbname = db_location)
...
clean_csv <- function(file, directory){
...
add_to_db <- function(file, directory = db_location, con = con){
...
file.size("Cann_ABM_GSA_1.sqlite3")

if(file.size(db_location) < 4000000){
